...
void RecordManager::Select(SQLSelect& st) {
	...
	for (int i = 0; i < tbl->GetAttributeNum(); ++i) {
		...
		attr_names.push_back(tbl->ats()[i].attr_name());
	}
	cout << endl;
	...
	for (int i = 0; i < tkey_values.size(); ++i) {
		for (int j = 0; j < tkey_values[i].size(); ++j) {
			...
			if (tkey_values[i][j].key_type() == 0) {
				int temp;
				tkey_values[i][j].GetValue(temp);
				record[attr_names[j]] = temp;
			}
			else if (tkey_values[i][j].key_type() == 1) {
				float temp;
				tkey_values[i][j].GetValue(temp);
				record[attr_names[j]] = temp;
			}
...
std::string RecordManager::GetRes()
{
	return res;
}

int RecordManager::GetStatusID()
{
	return statusID;
}
