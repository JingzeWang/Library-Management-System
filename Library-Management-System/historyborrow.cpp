...
#include "ui_historyborrow.h"
#include "widget.h"
#include <QMessageBox>
#include "testh.h"

historyborrow::historyborrow(QWidget* parent) :
	QDialog(parent),
	ui(new Ui::historyborrow)
{
	ui->setupUi(this);
	//
	...
	//补充：导入数据库里的借阅历史？？？
	/*
	ui->listWidget->insertItem(0,tr("序号   ID   书名   借阅时间   归还时间"));
	...
	*/
	...
	//?searchHistory(string id)

	std::vector<std::string> idNums;
	int result;

	//修改添加int searchHistory(string id, vector<string>&idNums)
	result = searchHistory(ID0.toStdString(), idNums);
	if (result == 0)
	{
		QMessageBox m;
	...
	else
	{
		...
		ui->tableWidget->setHorizontalHeaderLabels(QStringList() << "ID" << "书名" << "作者" << "书籍ISBN");// << "借阅时间" << "归还时间");
		...
		for (int i = 0; i < idNums.size(); i++)
		{
			...
			else
			{
				...
				ui->tableWidget->setItem(n, 2, new QTableWidgetItem(QString::fromStdString(author)));

				ui->tableWidget->setItem(n, 3, new QTableWidgetItem(QString::fromStdString(isbn)));
				//借阅时间：
